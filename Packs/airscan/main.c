...
static void pretty_print_network(WiFiNetworkRef network)
{
	...
	CFStringRef isWPA = (WiFiNetworkIsWPA(network) ? CFSTR("YES\n\n") : CFSTR("NO\n\n"));

	CFStringRef format = CFStringCreateWithFormat(kCFAllocatorDefault, NULL, CFSTR("%s%-40s%s BSSI: %s, RSSI: %.0f dBm, BARS: %d, CHANNEL: %d, HIDDEN: %@, EAP: %@, WEP: %@, WPA: %@"), "\x1B[01;36m", SSID, kNRM, BSSID, strength, bars, channel, isHidden, isEAP, isWEP, isWPA);
	CFShow(format);
	CFRelease(format);
}
